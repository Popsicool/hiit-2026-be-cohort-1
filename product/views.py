...
from product.serializers import ProductSerializer, BulkProductSerializer, SaleSerializer
...
class BulkProductSerializer(generics.GenericAPIView):
    serializer_class = BulkProductSerializer
    permission_classes = [permissions.IsAuthenticated]
    def post(self, request):
        serializer = self.serializer_class(data=request.data)
        serializer.is_valid(raise_exception=True)
        product_ids = serializer.validated_data.get("ids")
    ...
    def post(self, request):
        ...
        for buy_option in products:
            ...
            if prd.quantity < quantity:
                return Response(data={"message": "quantity more than what is available"}, status=400)
